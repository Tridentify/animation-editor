{//Spatium v0.1, by Ocelote
    ...
    let drawPolygon2 = (parr, opts) => {
        ...
        let { fill = "#0000", border = "#000f", width = 0.4 } = opts || {}
        ctx.beginPath()
    ...
    Spatium.mesh.transform.rotate = (faces, rotateX, rotateY, rotateZ, aroundX, aroundY, aroundZ) => faces.map(({ parr, opts }) => ({ parr: parr.map(([px, py, pz]) => addVec(rotx(roty(rotz([px - aroundX, py - aroundY, pz - aroundZ], rotateZ), -rotateY), -rotateX), [aroundX, aroundY, aroundZ])), opts }))
